...
int game_over_menu_render(GAME *game) {
  ...
  status = get_text_size(game->font, title[g_game_win], game->field.title_font,
  ...
  status =
      render_text(game->renderer, tex_game_result[g_game_win],
  ...
  status = get_text_size(game->font, game->game_over_menu->items[0].text,
  ...
  for (int i = 0; i < game->game_over_menu->count; i++) {
    ...
    if (i == game->game_over_menu->selected_item) {
      ...
      status = get_text_size(game->font, game->game_over_menu->items[i].text,
    ...
    status = render_text(game->renderer, item, &dst_rect);
    if (status == 0) {
      SDL_Log("render_text failed...\n");
      return 0;
    }
...
SCENE game_over_menu_scene = {.handle_events = game_over_menu_event_handler,
