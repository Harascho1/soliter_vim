...
void
free_resurses(char** array_of_strings) {
    for (int i = 0; i < 10; i++) {
...
int make_string_array(char **array_of_strings, int *i) {
    ...
    while (*i < 10 && ptr != EOF) {
        ...
        if (ptr == '\0') {
            ...
            status = fread(array_of_strings[(*i)++], sizeof(char), n_size, saves_files_bin);
            if (status == 0) {
                return 0;
            }
            prev_index = index + 1;
...
int
get_new_insert_index(int *array, int *i, int num) {
    int new_insert;
    for (int j = 0; j < 10; j++) {
        SDL_Log("[%d] broj je: [%d]\n", j, array[j]);
        if (array[j] == 0) {
...
int
make_new_array_of_strings(char **strings_of_array, int i, char *new_string, int new_insert_index) {
    char tmp[255];
...
int
print_in_bin(char **strings_of_array, int i) {
