...
int load_field(
  ...
  int status =
    get_text_size(font, "soVIMter", field->title_font, &tmp_text_width, &tmp_text_height);
  if (status == 0) {
  ...
  status =
    get_text_size(font, "soVIMter", field->item_font, &tmp_text_width, &tmp_text_height);
  if (status == 0) {
    SDL_Log("get_text_size error...\n");
    return 0;
  }
  ...
  status =
    get_text_size(font, "soVIMter", field->text_font, &tmp_text_width, &tmp_text_height);
  if (status == 0) {
    SDL_Log("get_text_size error...\n");
    return 0;
  }
