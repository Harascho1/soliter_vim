...
void change_cursor_frame(const GAME* game) {
  ...
  if (card != NULL) {
    SDL_Log("Cursor is on top of: %d", card->value);
    SDL_Log(
      "game cursor location in rows and cols: [%d %d]", game->cursor->pos->row,
      game->cursor->pos->col
    );
  }
  ...
  SDL_Log(
    "game cursor location in pixels: [%.1F %.1F]", game->cursor->cursor->x,
